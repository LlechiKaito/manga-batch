...
目次ページが見つからない場合は {"episodes": []} と返してください。`,
  ...
  const parsed = JSON.parse(jsonMatch[0]) as { episodes: Episode[] };
...
export async function detectEpisodeBoundaries(
  ...
  const boundaries: { episodeNumber: number; startPageIndex: number }[] = [];

  // 10ページずつバッチで処理
  for (let i = 0; i < pages.length; i += BATCH_SIZE) {
    const batch = pages.slice(i, i + BATCH_SIZE);
    console.log(
      `ページ境界検出中: ${i + 1}〜${i + batch.length} / ${pages.length}`
    );
